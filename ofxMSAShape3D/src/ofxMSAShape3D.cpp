...
ofxMSAShape3D::ofxMSAShape3D() {
	reservedSize = DEFAULT_RESERVE_AMOUNT;
	reset();
}

void ofxMSAShape3D::reserve(int reservedSize) {
...
void ofxMSAShape3D::glBegin(GLenum drawMode) {
...
void ofxMSAShape3D::draw() {
	glEnd();
}



void ofxMSAShape3D::glVertex(float x, float y, float z) {
...
void ofxMSAShape3D::glVertex3v(float *v) {
	glVertex(v[0], v[1], v[2]);
}

void ofxMSAShape3D::glVertex2v(float *v) {
	glVertex(v[0], v[1]);
}


void ofxMSAShape3D::glNormal(float x, float y, float z) {
	glNormal3f(x, y, z);
	normalChanged = true;
	curNormal[0] = x;
	curNormal[1] = y;
...
void ofxMSAShape3D::glNormal3v(float *v) {
...
void ofxMSAShape3D::glColor(float r, float g, float b, float a){
	glColor4f(r, g, b, a);
	colorChanged = true;
	curColor[0] = r;
	curColor[1] = g;
...
void ofxMSAShape3D::glColor(int hexColor){
...
void ofxMSAShape3D::glColor3v(float *v) {
	glColor(v[0], v[1], v[2]);
}

void ofxMSAShape3D::glColor4v(float *v) {
	glColor(v[0], v[1], v[2], v[3]);
}



void ofxMSAShape3D::glTexCoord(float u, float v) {
	curTexCoord[0] = u;
	curTexCoord[1] = v;
	texCoordChanged = true;
}

void ofxMSAShape3D::glTexCoord2v(float *v) {
	glTexCoord(v[0], v[1]);
}


void ofxMSAShape3D::glRect(float x1, float y1, float x2, float y2) {
