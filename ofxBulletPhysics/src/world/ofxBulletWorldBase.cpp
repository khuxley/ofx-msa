...
ofxBulletWorldBase::ofxBulletWorldBase() {
	bulletWorld				= NULL;
	solver					= NULL;
	broadphase				= NULL;
	dispatcher				= NULL;
...
void ofxBulletWorldBase::setup() {
	setup(ofPoint(-10000,-10000,-10000), ofPoint(10000, 10000, 10000));
}

void ofxBulletWorldBase::setup(ofPoint worldMin, ofPoint worldMax) {
...
void ofxBulletWorldBase::destroy() {
	...
	for (int i=0; i<objects.size(); i++) {
		ofxBulletRigidBody* o = objects[i];
		objects[i] = 0;
...
//
//
//ofxBulletObjectBox *createBox(ofPoint &position) {
//	ofxBulletObjectBox *box = new ofxBulletObjectBox;
//}
